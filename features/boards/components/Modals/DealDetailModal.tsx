...
export const DealDetailModal: React.FC<DealDetailModalProps> = ({ dealId, isOpen, onClose }) => {
  ...
  // Cliques repetidos em "Telefonar" (ex.: desktop sem handler de tel:) não podem
  // gerar N registos — 1 registo por janela de 2 minutos chega para o CHQ.
  const lastCallLogRef = React.useRef(0);
  const logCallOnce = () => {
    const now = Date.now();
    if (now - lastCallLogRef.current < 120000) return;
    lastCallLogRef.current = now;
    logCHQ('call');
  };
  ...
  const execActionWhatsApp = async () => {
    const d: any = deal;
    const phone: string = d.contactPhone ? String(d.contactPhone).replace(/[^0-9]/g, '') : '';
    if (!phone) { alert('Adiciona o telefone do contacto primeiro.'); return; }
    setExecLoading('wa');
    try {
  ...
  const execActionEmail = async () => {
    const d: any = deal;
    const email: string = d.contactEmail || '';
    if (!email) { alert('Adiciona o email do contacto primeiro.'); return; }
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl p-5">
                    ...
                    <div className="flex gap-3 items-center justify-start flex-wrap">
                      ...
                      {/* Sem logCHQ aqui: o pipeline de processamento da gravação já cria o
                          registo CALL quando o upload é real — registar ao abrir o modal
                          criava fantasmas (abrir e cancelar contava como chamada). */}
