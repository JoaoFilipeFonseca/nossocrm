...
/**
 * Central de I.A. (página inteira em `/ai`).
 *
 * Passou a renderizar a MESMA `UIChat` do "Foco Imo Pilot" (a estrela do topo),
 * agora em ecrã cheio: ganha markdown, aprovação de acções (mover/criar/…) e
 * contexto — e desaparece a duplicação de uma interface de chat própria e mais
 * fraca. Um só assistente em todo o CRM (arrumação do CRM, seguimento do W6).
 */
export const AIHubPage: React.FC = () => {
  const { data: settings } = useOrgSettings();
  const hasApiKey = Boolean(settings?.aiKeyConfigured);

  // Se não tem API key, mostra o ecrã de configuração (mais claro que um erro cru).
  if (!hasApiKey) {
