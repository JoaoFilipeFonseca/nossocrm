...
const [isUserMenuOpen, setIsUserMenuOpen] = useState(false);
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-surface-bg bg-dots">
      ...
      >
        ...
        <nav className={`flex-1 min-h-0 overflow-y-auto p-4 space-y-2 flex flex-col ${sidebarCollapsed ? 'items-center px-2' : ''}`} aria-label="Navegação do sistema">
          ...
          <button
            ...
            className={`flex items-center rounded-lg border border-slate-200 dark:border-white/10 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors ${
              sidebarCollapsed ? 'justify-center h-10 w-10' : 'gap-2.5 px-3 py-2 w-full'
            }`}
          >
            ...
            {!sidebarCollapsed && (
              <>
                ...
                <kbd className="ml-auto text-[10px] font-mono px-1.5 py-0.5 rounded bg-slate-100 dark:bg-white/10 text-slate-400">Ctrl K</kbd>
              </>
            )}
          </button>
          ...
          <header className="h-16 glass border-b border-[var(--color-border-subtle)] flex items-center justify-between px-4 md:px-6 z-40 shrink-0" role="banner">
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                aria-label="Pesquisar"
                title="Pesquisar"
                className="md:hidden p-2 text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/10 rounded-full transition-all active:scale-95 focus-visible-ring"
              >
      ...
      {/* Pesquisa global (Ctrl+K) — disponível em qualquer página */}
      <GlobalSearchModal />
    </div>
  );
};
