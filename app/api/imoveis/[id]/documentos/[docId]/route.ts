...
export async function GET(_request: NextRequest, ctx: { params: Promise<{ id: string; docId: string }> }) {
  try {
    ...
    const { data: signed, error } = await supabase.storage
      ...
      .createSignedUrl(doc.storage_path, 60 * 10); // 10min

    if (error || !signed) return NextResponse.json({ message: error?.message ?? 'Erro signed URL' }, { status: 500 });

    return NextResponse.json({ url: signed.signedUrl, filename: doc.filename, mime_type: doc.mime_type });
  } catch (err) {
...
export async function DELETE(_request: NextRequest, ctx: { params: Promise<{ id: string; docId: string }> }) {
  try {
    ...
    if (doc?.storage_path) {
      await supabase.storage.from('imovel-documentos').remove([doc.storage_path]);
    }

    const { error } = await supabase.from('imovel_documentos').delete().eq('id', docId);
