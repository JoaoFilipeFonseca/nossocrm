...
- **CT-AUTO · Auto-preenchimento de campos + motor de follow-up "nunca perder uma lead"** `[EM CURSO]` `P1` (CAPTURE 01/06; modelo afinado com o João 08/06)
  ...
  **Spec v1 TRAVADA (a construir agora — João aprovou 08/06 "vamos assim, depois ajustamos"):**
   - **Config (organization_settings):** `followup_batch_size` (default 10), `followup_cooldown_days` (default 30), `followup_enabled` (default true).
   - **Estado no negócio (`deals.custom_fields`):** `snoozedUntil` (date) + `snoozeReason` (adiar); `followupQueuedOn` (date, dedup/cooldown da leva).
   - **RPC `deal_followups_due(p_org, p_batch, p_cooldown)`** (SECURITY DEFINER): elegíveis = abertos (não won/lost) + não adiados + fora do cooldown + sem toque recente; ordena por etapa(desc)+recência(desc); LIMIT batch. "Último toque" = max(deal_activity real, last_stage_change_date, created_at). Wrapper in-app passa `get_user_org_id()`.
   - **Edge `lead-followups`** (espelha `cmi-watch`: X-Cron-Secret, skip-domingos, por-org com Telegram): chama a RPC, cria tarefa (`activities`, type follow_up, deal+contact) por negócio, marca `followupQueuedOn=hoje`, envia digest Telegram clicável. **Nunca 500.** + **pg_cron `0 9 * * 1-6`** + linha em `system_automations` (key `lead-followups`, em /automacoes).
   - **UI "Adiar"** no DealDetailModal (6m/1ano/data → grava `snoozedUntil`), em vez de perder.
