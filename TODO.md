...
- **MA-LEADFORM · Criar o formulário de leads (instant form) da Meta a partir do CRM** `[POR FAZER]` `P?` (CAPTURE 03/06, ideia do João)
...
- **MA-CREATE · Construtor de anúncios de raiz no CRM ("Novo anúncio", à medida do João)** `[POR FAZER]` `P?` (CAPTURE 03/06, spec detalhada do João — é o Tier 4 do MA-EDIT-FULL)
  ...
  - 🚨 **DEPENDÊNCIA CRÍTICA (a confirmar, não assumir):** criar campanha/conjunto/anúncio via Marketing API **pode exigir o "Marketing API Access Tier = Full access"** (Business verification + App Review) — o **mesmo portão do vídeo**. MAS a criação de **criativos de imagem já funciona** em Limited → criar campanhas/anúncios PODE funcionar também (testar com um probe PAUSED antes de construir o builder inteiro). **CAPI** e **criar formulário** têm gates próprios (CAPI=setup dataset/token; form=`pages_manage_ads`, já tratado no MA-LEADFORM). **Sequência inteligente:** (1) de-riscar o gate de criação; (2) se gated → upgrade do tier primeiro; (3) construir o builder por fases com maqueta. Reusa `lib/integrations/meta/{write,server,leadforms}.ts` + `audit_logs`.
