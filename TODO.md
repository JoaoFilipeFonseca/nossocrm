...
## 🗓️ Registo da sessão 17 Jul 2026 — Primeiros dias de uso real: registos fantasma de chamada + apagar histórico
**Relato do João (a ligar a leads):** "tento ligar não liga mas regista e depois não permite apagar; gravação não está a fazer".
1. **Registo fantasma por clique em Telefonar:** cada clique gravava 2 registos às cegas (CHQ em `deal_activities` + tarefa "Chamada iniciada" em `activities`), mesmo sem chamada (desktop sem handler de `tel:`). Corrigido: 1 só registo CHQ com debounce de 2 min; tarefa duplicada removida.
2. **Botão "Adicionar chamada" registava ao ABRIR o modal** (antes de existir gravação; cancelar contava como chamada) e o pipeline de processamento regista outra vez no fim = duplicado. Corrigido: só o pipeline regista, quando o upload é real.
3. **Histórico de interacções não deixava apagar:** só entradas `via=timeline-manual` eram apagáveis; os toques dos botões (via=deal-detail-modal) ficavam presos. Corrigido: TODA a entrada com `actor='human'` é apagável (UI + API com validação de âmbito do contacto); automação/sistema nunca. Lixo visível também em mobile (antes só aparecia em hover).
- **TELEFONIA-GRAVACAO · "carrego em ligar e grava automaticamente"** `[PARCIAL]` (pedido João 17/07; João escolheu na hora: iPhone + Notta via Atalho iOS)
  ✅ **Lado CRM FEITO 17/07:** `lib/calls` (isIOS/telHref/shortcutCallHref/interceptCallClick + testes) + `CallLink`; aplicado nas 4 superfícies de chamada (modal do negócio, ficha do contacto ×2, lista de contactos, Power List /hoje). Em iOS o clique dispara o Atalho **"Ligar CRM"** (`shortcuts://run-shortcut`, número como input); fora do iOS mantém `tel:`. Receita completa do Atalho (Notta "iniciar gravação" → Ligar) em `docs/ligar-com-gravacao-notta.md`, com nota legal (consentimento) e limitação iOS (gravação por microfone ⇒ alta-voz).
  ⏳ **Pendente João (só ele pode):** criar o Atalho "Ligar CRM" no iPhone (2 min, receita no doc) e testar uma chamada real. ⏳ Futuro possível já capturado: cola-URL da Notta → transcrição automática no CRM (caminho canónico em `memory/reference_notta_ai.md`).
...
- **MSG-3 · Email — evolução do Resend** `[EM CURSO — inbound FEITO 23/07]` `P?`
  ...
  **✅ INBOUND FEITO E VERIFICADO (23/07, task_3ebe04f0):** a edge `messaging-webhook-resend` trata `email.received` de 2 fontes — (A) Cloudflare Email Worker (`?source=worker`, auth `credentials.inboundSecret`) e (B) Resend Inbound (Svix + `GET /emails/receiving/{id}` best-effort para o corpo). Associa por `In-Reply-To`/`References` → `nurture_emails`/`messaging_messages` (negócio exacto) ou pelo remetente → contacto → negócio aberto; grava `messaging_message` inbound + `deal_activity` (`type=email_inbound`, `actor=system`) na timeline; marca `nurture_emails.replied_at`. **Nunca cria lead** (contacto≠lead), idempotente (dedup `messaging_webhook_events` + `external_id` único), nunca 5xx. Registado em **/automacoes** como webhook `email-inbound` (coluna nova `system_automations.kind='webhook'`; RPCs saltam pg_cron; ON/OFF real lido pela edge; UI mostra "dispara a cada email"). Migração `20260723120000`. Edge deploy v3 (verify_jwt:false). Verificado por curl: default-deny 401, auth worker 401 s/ segredo, desconhecido→sem lead, conhecido→msg+timeline+negócio, In-Reply-To nurture→replied_at, idempotência duplicate, OFF→skipped. Artefactos de teste limpos.
  **⏳ PENDENTE (mão do João — único):** configurar o inbound do correio → `docs/email-inbound-cloudflare-checklist.md`. Recomendado **Opção A (Cloudflare Email Worker, `docs/email-inbound-worker.js`)**: aditivo, mantém reply-to e a caixa do João, só acrescenta a cópia ao CRM; colar o Worker + secret `CRM_INBOUND_SECRET` (o `inboundSecret` já gravado no canal). Alternativa **Opção B (Resend Inbound)**: subdomínio `reply.` + MX + reply-to + webhook `email.received` + `webhookSecret` Svix (este secret também destrava o tracking open/click do nurture — BRIEF 7).
  **Falta ainda (fora do inbound):** bounce/complaint já mapeados a `failed`; open/click do nurture dependem do `webhookSecret` (mão do João).
...
- **MA-ROUTING-UX · Encaminhamento de leads por campanha — lista não pode ficar gigante (arquivar, não apagar)** `[FEITO 23/07]` `P1` (CAPTURE 23/07, pedido do João no ecrã /anuncios)
  **✅ FEITO E VERIFICADO (23/07):** painel `LeadRoutingPanel` (/anuncios) ganhou chips **Activas · Por definir · Arquivadas** com contadores; vista por defeito só activas+por-definir (as inactivas já com destino ficam escondidas). Acção **Arquivar/Desarquivar** por linha (não apaga). **Auto-reaparecimento** no GET: campanha arquivada que fica activa (gasto/entrega ≤7 dias em `ad_insights`) OU recebe lead nova (`contacts.attribution->>campaign_id`, `created_at` > arquivo) limpa o flag e volta a "por definir". Flag de arquivo em tabela própria `meta_campaign_archive` (RLS org, migração `20260723130000`). RPC `meta_campaign_routing_overview` (migração `20260723130500`) agrega destino+activa+última lead+arquivo sem limite de linhas. Endpoint `POST /api/meta-ads/routing/archive` (admin+org, nunca 5xx). Badge "activa" (ponto verde). Sem novo item na barra lateral. Verificado em produção (git push→Vercel, desktop+375, 0 erros consola). Dados reais do João: 2 activas (T2 Rua da Alegria, T3 Leça, ambas com destino), ~10 inactivas por definir a entulhar → agora arquiváveis.
  **Problema (palavras do João, com print):** o painel "Encaminhamento de leads (por campanha)" no `/anuncios` lista TODAS as campanhas Meta para lhes dar destino (board+etapa). Já tem "9 por definir" e vai crescer sem fim → paredão impossível de gerir. "tenho que poder minimizar ou escolher apenas os que estão ativos ou apagar mas nunca ficar com uma lista gigante".
  **Requisito exacto (a subtileza que ele frisou):** "imagina que apago, se ele ficar ativo aparece de novo aqui" → **apagar = ARQUIVAR, nunca destruir**. Se uma campanha arquivada voltar a ficar activa (ou receber uma lead nova), tem de **REAPARECER automaticamente** como "por definir" — senão os leads dessa campanha entram sem destino e perdem-se.
  **Spec:**
  1. **Filtro/segmentos** no topo do painel: **Activas · Por definir · Arquivadas** (contadores). Vista por defeito = só as que precisam de atenção (activas + por definir), NÃO tudo.
  2. **Acção "Arquivar"** por linha (substitui a ideia de "apagar") → sai da vista principal para "Arquivadas". Reversível (desarquivar manual).
  3. **Auto-reaparecimento:** campanha arquivada que passe a activa na Meta OU receba lead nova volta sozinha a "Por definir" (limpa o flag de arquivo). É a regra de segurança — nunca perder o encaminhamento de uma campanha viva.
  4. "Activa" = tem gasto/entrega recente ou estado ACTIVE na Meta (reusar sinais que o /anuncios já lê); definir a janela (ex.: activa nos últimos N dias).
  5. Persistir o estado de arquivo por org (coluna/flag; RLS; multi-tenant). Sem novo item na barra lateral — é dentro do painel que já existe.
  **Aceitação:** lista por defeito só mostra activas+por-definir; arquivar tira da vista; campanha arquivada que fica activa reaparece automaticamente em "por definir"; contadores certos; desktop+mobile 375.
