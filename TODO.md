...
- **MA-CREATE-PUBLICAR · Passo de publicação no fim do construtor (Pausa vs Publicar, com confirmação forte)** `[POR FAZER]` `P1` (CAPTURE 04/06, pedido do João — "eu quero publicar; posso deixar em pausa ou publicar, com um passo extra do tipo 'tens mesmo a certeza, está tudo bem?'")
  Hoje o builder cria SEMPRE tudo em pausa (campanha+conjunto+anúncio PAUSED; formulário DRAFT). O João quer poder **publicar a sério** a partir do CRM, com uma **confirmação forte** antes (gasta dinheiro real). **Buildável:** no ecrã final, escolha **Deixar em pausa** (default) ou **Publicar agora**; se Publicar → diálogo de confirmação que mostra o que vai ligar e o **gasto previsto** (orçamento/dia) e pede confirmação explícita; ao confirmar, pôr **ACTIVE** no anúncio + conjunto + campanha (`setAdStatus`/`setBudget` já existem; falta activar conjunto/campanha) **e activar o formulário** (DRAFT→ACTIVE) porque um anúncio de leads não corre com o form em rascunho. ⚠️ **Acção outward + dinheiro real:** confirmação dupla obrigatória; na verificação NUNCA deixar um anúncio activo a gastar (publicar+repausar imediatamente, ou validar só o diálogo). Aprendizado a confirmar: a Meta pode exigir revisão antes de entregar. Liga ao analista IA (medir depois). Reusa `lib/integrations/meta/write.ts` + audit.
