...
- **SOCIAL-INBOX · Avisos de comentários/DMs do Facebook e Instagram que precisam de acção (IA prepara, João envia)** `[EM CURSO — de-risco feito]` `P1` (CAPTURE 03/06, ideia do João — "muito importante"; nº 2 da sequência travada)
  ...
  **🔎 DE-RISCO FEITO (08/06, Graph API com o token actual):** ✅ infra existe — a Página "João Fonseca" `104774959239895` tem o papel **MESSAGING** e tem **Instagram Business ligado** (`@jfonseca.pt`, id `17841458753844552`). ❌ **o token actual NÃO tem as permissões de mensagens** (tem: pages_show_list, ads_management, ads_read, business_management, leads_retrieval, pages_read_engagement, pages_manage_metadata, pages_manage_ads). **FALTAM:** `pages_messaging` (Messenger), `instagram_manage_messages` + `instagram_basic` (IG DM). **Gate:** (1) eu acrescento estes scopes ao OAuth (`config.ts`), (2) o João **re-autoriza** a Meta, (3) `pages_messaging`/`instagram_manage_messages` são **avançadas → App Review provável** (verificação de negócio + submissão) para uso live; em **modo Desenvolvimento** dá para construir+testar com a conta do João (admin), como nas leads. **Estratégia (a confirmar com o João): de-risco do API real primeiro** (com a conta dele, modo dev) antes de construir a UI — espelha o de-risco do MA-CREATE/vídeo. Webhook: campo `messages`/`messaging_postbacks` (Messenger) + `messages` do IG; janela de resposta 24h. **Integração Meta `79df5ad9-5a6c-441c-b27c-111197a42bab`; token no Vault `meta_oauth_token_79df5ad9-...`.**
