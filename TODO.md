...
- **MA-EDIT-FULL · Gestão completa de anúncios no CRM (criar/editar/duplicar como na Meta)** `[EM CURSO — Tier 1 feito]` `P?` (CAPTURE 03/06, ideia do João)
  ...
  **Falta (próximos tiers):** (b) **Tier 2 imagem/vídeo** (upload media p/ Meta + criativo novo + swap); (c) **Tier 3 MA-DUPLICATE** (duplicar anúncio/conjunto/campanha A/B, reusando o existente — fluxo seguro Meta: duplicar o vencedor e editar a cópia); (d) **Tier 4 MA-CREATE/builder** (campanha→conjunto→anúncio de raiz; sobrepõe-se a MA-CREATE do [[MKT-STUDIO]]); (e) **MA-EDIT-DYNAMIC** (editar `asset_feed_spec`). Reusa `lib/integrations/meta/{write,server}.ts`, `audit_logs`, Brand Kit + `/criativos`. Sempre confirmação + registo. Teste "serve qualquer consultor".
