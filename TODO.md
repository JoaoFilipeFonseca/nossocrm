...
- **MKT-SEQUENCES · Sequências de email (drip) nas automações** `[POR FAZER]` `P?` (CAPTURE 11/06, ideia do João — "tenho que ter as minhas sequências de emails, talvez automações em que crio um conjunto específico: boas-vindas, passado 3 horas sai outro, passado 1 dia sai outro…")
  Sequência = conjunto nomeado de emails com **esperas relativas** (0h boas-vindas → +3h → +1 dia → …), disparado por evento (ex.: lead nova) e a viver em `/automacoes` (regra: toda a automação aparece lá). **Reusa o que já existe:** canal Resend LIVE, átomo `action.send_email`, motor com esperas (`automation-resume` tick) e eventos (`automation-event-listener`, evento `lead.meta_ads`), tom do João/Brand Kit. **RGPD obrigatório no rodapé de cada email: link de anular subscrição + política de privacidade** (liga ao pedido do João 11/06 sobre política de privacidade), UTF-8 sempre, nunca Domingos, PT-PT pré-AO.
