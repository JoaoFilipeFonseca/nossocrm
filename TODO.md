...
- **NS-2 · CONTACT-360-AI — perfil 360° + IA que relaciona tudo** `[EM CURSO]` `P1`
  ...
  **Fase 2 FEITA e LIVE (01/06):** bloco "Sugestões para a ficha" no Assistente 360 — a IA propõe campos com evidência (DISC/triggers/trimestre/família/animais/morada), Aceitar grava em `custom_fields` (`POST /api/contacts/[id]/enrich`, merge server-side, triggers em união) e reflecte logo em "Sobre a pessoa" (ContactRichPanel re-sincroniza com props), Ignorar descarta. Verificado live (aceitar família → gravou + apareceu). **Falta:** Fase 3 = **memória/aprendizagem** (guardar análises em `contact_ai_analyses`, histórico, registar aceites/ignorados para aprender, ciclo resulta→continua).
  --- visão original ---
