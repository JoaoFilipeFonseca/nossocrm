...
- **MA-EDIT-FULL · Gestão completa de anúncios no CRM (criar/editar/duplicar como na Meta)** `[EM CURSO — Tier 1 feito]` `P?` (CAPTURE 03/06, ideia do João)
  ...
  **Falta (próximo tier):** (b) **Tier 2 imagem/vídeo** (upload de media p/ Meta + criativo novo + swap — único que falta do pipeline de criativo); (d) **Tier 4 MA-CREATE/builder** (campanha→conjunto→anúncio de raiz; sobrepõe-se a MA-CREATE do [[MKT-STUDIO]]). Reusa `lib/integrations/meta/{write,server}.ts`, `audit_logs`, Brand Kit + `/criativos`. Sempre confirmação + registo.
