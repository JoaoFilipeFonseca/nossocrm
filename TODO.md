...
- **CAPTURE — TELEFONIA-GRAVACAO · "carrego em ligar e grava automaticamente"** `[POR FAZER]` `P?` (pedido João 17/07): impossível gravar chamada GSM a partir do browser — a chamada acontece no telemóvel, fora do CRM. Caminhos reais para decidir: (a) **Notta melhor integrado** (ver `memory/reference_notta_ai.md`: cola-URL-pública ~1h esforço, iOS Shortcut, email-forward; Zapier requer upgrade Business ~29$/mês); (b) **telefonia integrada no CRM** (Twilio Voice/CloudTalk/etc.): clicar liga MESMO pelo browser/app com gravação automática — número novo (~5-25€/mês + minutos), aviso legal de gravação obrigatório em PT, projecto de 1-2 sessões; (c) desistir da gravação e ficar só com o registo CHQ + notas ditadas (botão Ditar já existe). Decisão do João; nada construído.
