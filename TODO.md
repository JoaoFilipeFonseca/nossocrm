...
- **MA-EDIT-FULL · Gestão completa de anúncios no CRM (criar/editar/duplicar como na Meta)** `[POR FAZER]` `P?` (CAPTURE 03/06, ideia do João)
  Visão do João: fazer no CRM **tudo o que a Meta permite** para montar e gerir anúncios, sem abrir o Ads Manager. **Já feito (MA-EDIT tier fácil):** ligar/desligar (pausar/reactivar) + mudar orçamento (valor) do conjunto/campanha. **Falta (épico, por tiers, com risco — escreve criativos na Meta):** (a) **MA-EDIT médio** — mudar a **copy/texto** do anúncio (criar criativo novo c/ texto novo + trocar); (b) **MA-EDIT difícil** — mudar a **imagem/vídeo** (upload p/ Meta + novo criativo + swap); (c) **MA-DUPLICATE** — **duplicar** anúncio/conjunto/campanha para testar outra versão (A/B), reusando o que já existe; (d) **MA-CREATE/builder** — **construtor de raiz** (campanha→conjunto→anúncio: objectivo, público, orçamento, posicionamentos, criativo) — sobrepõe-se ao sub-épico MA-CREATE do [[MKT-STUDIO]]. Reusa `lib/integrations/meta/{write,server}.ts`, `audit_logs`, Brand Kit + `/criativos` (criativos IA). Sempre confirmação + registo. Teste "serve qualquer consultor". **Decidir tiers/ordem e escopo MVP com o João antes de arrancar.**
