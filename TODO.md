...
- **IMO-6 · CMI (Contrato de Mediação Imobiliária): validade + contagem decrescente + acções** `[EM CURSO]` `P1` (CAPTURE 01/06; corrigido 02/06: é **CMI**, NÃO "mandato" — o João reserva "mandato" para o lado do comprador)
  ...
  **Falta Fase 2b (P1, a seguir):** **automação periódica** (`/automacoes`, [[regra-automacoes-no-crm]]) que, perante **falta de negócios/visitas/propostas** e proximidade do fim do CMI, cria **tarefa + alerta Telegram** com acção/estratégia sugerida (liga a **IMO-3** "imóvel parado → alerta IA"). Hoje `deals.imovel_id` está a 0 → o KPI Negócios só enche quando se passarem a ligar negócios a imóveis (capturar/ligar a CT/board).
