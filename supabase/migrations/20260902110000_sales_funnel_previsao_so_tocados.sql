-- RESET-1 (parte 2) — a previsão do Funil de Vendas passa a obedecer à mesma
-- regra do Painel Diário: só entra no valor em aberto o negócio onde houve um
-- toque HUMANO a partir de organization_settings.counters_reset_at.
--
-- Porquê: `open_value` somava `deals.value` de TODOS os negócios abertos. Com a
-- base de proprietários importada (que traz o preço do imóvel como valor do
-- negócio), o Funil anunciava dezenas de milhões em "aberto" sem uma única
-- chamada feita. O número de negócios (`open_deals`) mantém-se — isso é um
-- facto; o valor previsto é que era mentira.
--
-- Só muda o bloco de `v_open_value`. Todo o resto da função fica igual.
...
begin
  ...
  select counters_reset_at into v_reset
  from organization_settings where organization_id = v_org;

  -- RESET-1: contagem = todos os abertos (facto). Valor = só os que eu toquei.
  select count(*)::int,
         coalesce(sum(coalesce(d.value,0)) filter (
           where exists (
             select 1 from deal_activities da
             where da.deal_id = d.id
               and da.actor = 'human'
               and (v_reset is null or da.created_at >= v_reset)
           )
         ), 0)
