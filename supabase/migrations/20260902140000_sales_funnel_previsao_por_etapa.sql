-- REGRA DE OURO — a previsao do Funil segue a etapa, nao o toque humano.
-- Substitui o criterio de counters_reset_at (migracao 20260902110000, mesma
-- manha): agora que existem Prospect e Contactos como etapas de espera, estar
-- fora delas E o sinal. Mais simples e auto-suficiente: mover para Oportunidade
-- — que ja exige valor estimado, por trigger — e o acto que faz o negocio
-- passar a contar.
...
begin
  ...
  -- Contagem = todos os abertos (facto). Valor = so o que esta no funil.
  select count(*)::int,
         coalesce(sum(coalesce(d.value,0)) filter (
           where coalesce(bs.excludes_followup, false) = false
         ), 0)
