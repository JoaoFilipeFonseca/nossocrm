-- Meta Ads Fase B (b2) — fix da RPC meta_ads_performance.
-- sum(bigint) devolve numeric; as colunas impressions/clicks estavam declaradas
-- bigint → erro "structure of query does not match function result type".
-- Cast explícito de sum(impressions)/sum(clicks) para bigint.
