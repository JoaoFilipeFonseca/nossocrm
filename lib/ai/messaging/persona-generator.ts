 ...
 * Usado no onboarding UX (Ecrã 1 e 2).
 *
...
export async function generatePersonaPrompt({
  ...
  const { text } = await generateText({
    model,
    prompt: `É um especialista em criar personas de IA para agentes de vendas.
Com base no contexto de negócio e objectivo abaixo, gere um system prompt completo para um agente de vendas.
...
2. Explica o objectivo principal de forma clara
...
/**
 * Sugere handoff keywords com base no objectivo do agente.
 */
export async function suggestHandoffKeywords({
  ...
  const { text } = await generateText({
    model,
    prompt: `Com base neste objectivo de agente de vendas:
"${agentGoal}"

Sugira 5-8 palavras ou frases que, quando ditas pelo lead, indicam que quer falar com um humano.
Exemplos: "falar com assistente", "humano", "responsável"
