...
export const ATOM_CATALOG: readonly AtomMetadata[] = [
  ...
  },
  {
    ...
    icon: '💬',
    description: 'Envia mensagem de texto via canal WhatsApp Meta Cloud connected da organização.',
    configSchema: {
      type: 'object',
      properties: {
        to: { type: 'string', description: 'Número destino com indicativo (ex: 351912345678).' },
        text: { type: 'string', description: 'Corpo da mensagem. Suporta {{...}}.' },
        channel_id: { type: 'string', description: 'Opcional. UUID do canal. Default = 1º canal Meta Cloud connected.' },
      },
      required: ['to', 'text'],
    },
  },
  {
    ...
    name: 'Enviar email',
    icon: '📧',
    description: 'Envia email via canal Resend connected da organização. Necessita DKIM/SPF configurados.',
    configSchema: {
      type: 'object',
      properties: {
