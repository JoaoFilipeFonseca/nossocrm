...
| **Varrimento de copy PT‑PT pré‑AO + brasileirismos (22/06 adiantado)** | grep whole‑word em features/+app/ → correcção de toda a copy visível + 1 teste | ✅ **commit `4415164`** — 25 componentes: Acção/Acções, Interacção/Interacções, Selecção, Objectivo, activo, reacções (pré‑AO); Tap→Toque, "Por que estou sugerindo isso?"→"Porque é que sugiro isto?", Acesse→Aceda, "Ana Souza"→"Ana Sousa", sobrenome→apelido, Você→Eu, essa→esta, seu→o seu. Teste US‑AI‑008 actualizado p/ "interacções". **typecheck 0 · lint 0 · vitest 558/5** (1 teste que afirmava a grafia antiga corrigido). Comentários de código e prompts de IA internos (não‑visíveis) ficam p/ depois; "Estágio"→"Etapa" (lexical pervasivo) capturado | **18 Jun** |
...
| 18 | **Negócio fantasma no Boards** — `dealsService.getAll` era a única superfície sem filtro `deleted_at IS NULL`; soft-deleted (limpeza QA de 15/07 por SQL) apareciam no kanban com "Sem contacto" | 16 Jul | `d55674a` | ✅ corrigido+verificado em produção; dados de teste purgados (0 soft-deleted na BD) |
| 19 | **Cliques mortos no ConfirmDialog dentro do DealDetailModal** — o focus-trap fazia `preventDefault+stopImmediatePropagation` em capture no document para cliques fora do seu DOM; o AlertDialog Radix monta em portal no body ⇒ Eliminar/Cancelar não faziam NADA (sem erro na consola). Impossível eliminar negócios | 16 Jul | `d55674a` | ✅ corrigido+verificado em produção (desktop clique físico + mobile 375 sequência de eventos): allowlist de portals no `FocusTrap.tsx` + trap suspenso com o diálogo aberto; toast de sucesso agora só após DELETE real |
| 20 | **Página Inicial das Configurações ignorada no arranque** — Configurações gravam `user_settings.default_route`; login lia `profiles.landing_page` (legado, nunca escrito) e a raiz `/` redirigia hardcoded para /dashboard; Perfil→Preferências era 3.ª fonte contraditória | 16 Jul | `d55674a`+`8416c66`+`189ba65` | ✅ corrigido+verificado em produção: login limpo aterra em /boards; Perfil unificado no mesmo campo com todas as rotas |
| 23 | **Todos os negócios "Sem contacto" (nome/email/telefone vazios no modal e na ficha do negócio)** — a DEALS_VIEW junta contactos com `contactsService.getByIds` num único `in()` do PostgREST; com 666 contactos o URL passa ~26KB → HTTP 400 → a junção falhava inteira. Rebentou quando a base cresceu (Radar Maia + imports); afectava TODAS as plataformas (o João notou no telemóvel). Provado com curl: 666 ids = 400 | 18 Jul | `a0b0335` | ✅ corrigido: `getByIds` (contacts + companies) em lotes de 100 com merge; verificado em produção a 375px e desktop |
| 21 | **Registos fantasma de chamada** — cada clique em "Telefonar" no modal do negócio gravava 2 registos às cegas (CHQ `deal_activities` + tarefa "Chamada iniciada" em `activities`) mesmo sem chamada acontecer (desktop sem handler `tel:`); o botão "Adicionar chamada" registava CHQ ao ABRIR o modal (cancelar contava como chamada; o pipeline da gravação regista outra vez no fim = duplicado). Encontrado pelo João no 1.º dia de uso real | 17 Jul | `67df3df` | ✅ corrigido+verificado em produção: 3 cliques → 1 só registo (debounce 2 min), 0 tarefas duplicadas; upload só regista via pipeline |
| 22 | **Histórico de interacções não deixava apagar registos dos botões de contacto** — a UI só mostrava o lixo e a API só apagava entradas `via=timeline-manual`; os toques via modal (`deal-detail-modal`) ficavam presos para sempre; lixo invisível em mobile (só hover) | 17 Jul | `67df3df` | ✅ corrigido+verificado em produção: toda a entrada `actor=human` é apagável (API valida âmbito do contacto; automação/sistema protegidas); lixo visível em mobile; apagar na UI remove da BD (confirmado) |
